...
class EdgeFlowFastCompiler:
    ...
    def _estimate_optimization_impact(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        quant_impact = self.quantization_impact.get(
            quantize, self.quantization_impact["none"]
        )

        # Device-specific adjustments
        device_chars = self.device_characteristics.get(
            device, self.device_characteristics["cpu"]
        )

        # Calculate estimated improvements
        size_reduction = quant_impact["size_reduction"]
        speed_improvement = (
            quant_impact["speed_improvement"] * device_chars["base_speed"]
        )
        memory_reduction = (
            quant_impact["memory_reduction"] * device_chars["memory_efficiency"]
        )
        ...
        if enable_pruning:
            ...
            )  # Up to 30% additional from pruning
            speed_improvement *= (
                1 + pruning_sparsity * 0.2
        ...
        return {
            ...
            "optimization_confidence": self._calculate_confidence(
                config, quant_impact, device_chars
            ),
        }
    ...
    def _calculate_confidence(
        self,
        config: Dict[str, Any],
        quant_impact: Dict[str, float],
        device_chars: Dict[str, float],
    ) -> float:
        ...
        if device == "raspberry_pi" and quantize == "int8":
            confidence += 0.15
        elif device in ["jetson_nano", "jetson_xavier"] and quantize in [
            "int8",
            "float16",
        ]:
            confidence += 0.1
    ...
    def _generate_warnings(
        ...
        if device == "cortex_m4" and quantize == "float16":
            warnings.append(
                "FLOAT16 quantization not supported on Cortex-M4 (no FP16 unit)"
            )

        if device in ["cortex_m4", "cortex_m7"] and memory_limit and memory_limit > 512:
            warnings.append(
                f"Memory limit ({memory_limit}MB) may be too high for {device}"
            )

        # Model format warnings
        if model_path.endswith((".pth", ".pt")) and quantize == "int8":
            warnings.append(
                "PyTorch models may need conversion to ONNX for optimal INT8 quantization"
            )
        ...
        if confidence < 0.6:
            warnings.append(
                "Low confidence in optimization estimates - results may vary significantly"
            )

        # Memory constraint warnings
        if memory_limit:
            estimated_size_reduction = estimated_impact.get(
                "estimated_size_reduction_percent", 0
            )
            if estimated_size_reduction < 50 and memory_limit < 256:
                warnings.append(
                    "Consider more aggressive optimization for tight memory constraints"
                )
    ...
    def compare_configurations(
        ...
        return {
            "config1": {
                ...
                "speed_improvement": impact1.get(
                    "estimated_speed_improvement_factor", 1.0
                ),
                "memory_reduction": impact1.get(
                    "estimated_memory_reduction_percent", 0
                ),
                "confidence": impact1.get("optimization_confidence", 0.8),
            },
            "config2": {
                "size_reduction": impact2.get("estimated_size_reduction_percent", 0),
                "speed_improvement": impact2.get(
                    "estimated_speed_improvement_factor", 1.0
                ),
                "memory_reduction": impact2.get(
                    "estimated_memory_reduction_percent", 0
                ),
                "confidence": impact2.get("optimization_confidence", 0.8),
            },
            "comparison": {
                "size_improvement": impact2.get("estimated_size_reduction_percent", 0)
                - impact1.get("estimated_size_reduction_percent", 0),
                "speed_improvement": impact2.get(
                    "estimated_speed_improvement_factor", 1.0
                )
                - impact1.get("estimated_speed_improvement_factor", 1.0),
                "memory_improvement": impact2.get(
                    "estimated_memory_reduction_percent", 0
                )
                - impact1.get("estimated_memory_reduction_percent", 0),
            },
            "recommendation": self._get_configuration_recommendation(impact1, impact2),
        }

    def _get_configuration_recommendation(
