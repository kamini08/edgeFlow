...
class DeviceProfile:
    ...
    def get_optimization_config(self) -> Dict[str, Any]:
        ...
        config = {
            ...
            "preferred_precision": "int8" if "int8" in self.preferred_quantization else "fp16",
        ...
        memory_constraint = self.get_constraint("total_memory_mb") or self.get_constraint("max_memory_mb")
        if memory_constraint and memory_constraint.value < 2048:  # Less than 2GB
        ...
        if cpu_cores and cpu_cores.value <= 4:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "DeviceProfile":
        ...
        data['constraints'] = [DeviceConstraint.from_dict(c) for c in data['constraints']]
        data['capabilities'] = [DeviceCapability.from_dict(c) for c in data['capabilities']]
...
class DeviceProfileManager:
    ...
    def _create_default_profiles(self) -> None:
        ...
        rpi4.add_constraint(ConstraintType.MEMORY, "max_memory_mb", 4096, "MB", description="Maximum available RAM")
        rpi4.add_constraint(ConstraintType.COMPUTE, "max_ops_per_sec", 5e8, "ops/sec", description="Approximate compute capacity")
        rpi4.add_constraint(ConstraintType.POWER, "max_power_watts", 15, "W", description="Maximum power consumption")
        rpi4.add_capability("arm_neon", True, description="ARM NEON SIMD instructions")
        rpi4.add_capability("gpu_acceleration", True, version="VideoCore VI", description="GPU acceleration support")
        ...
        mobile.add_capability("gpu_acceleration", True, description="Mobile GPU support")
        ...
        edge_tpu.add_constraint(ConstraintType.COMPUTE, "max_ops_per_sec", 4e12, "ops/sec")
        edge_tpu.add_constraint(ConstraintType.POWER, "max_power_watts", 2, "W")
        edge_tpu.add_capability("tpu_acceleration", True, version="Edge TPU", performance_score=95.0)
        ...
        server_gpu.add_constraint(ConstraintType.COMPUTE, "max_ops_per_sec", 1e14, "ops/sec")
    ...
    def list_profiles(self, category: Optional[DeviceCategory] = None) -> List[DeviceProfile]:
    ...
    def search_profiles(self, query: str) -> List[DeviceProfile]:
        ...
        for profile in self.profiles.values():
            if (query in profile.name.lower() or
                query in profile.manufacturer.lower() or
    ...
    def update_profile_constraint(
        ...
        **kwargs
        ...
        if constraint:
            ...
            profile.add_constraint(constraint_type, constraint_name, new_value, unit, **kwargs)
        
        profile.updated_at = datetime.now()
        self._save_profile(profile)
        return True
    
    def benchmark_device(self, device_id: str, benchmark_results: Dict[str, float]) -> None:
    ...
    def auto_detect_device(self) -> Optional[DeviceProfile]:
        ...
        for profile in self.profiles.values():
            ...
            if "raspberry" in system_info["platform"].lower() and "raspberry" in profile.name.lower():
        ...
        profile.add_constraint(
            ...
            "max_memory_mb",
            int(system_info["memory_gb"] * 1024 * 0.8),  # 80% of available memory
            "MB",
    ...
    def export_profiles(self, output_file: str) -> None:
        ...
        export_data = {
            ...
            "profiles": {pid: profile.to_dict() for pid, profile in self.profiles.items()},
        }
...
if __name__ == "__main__":
    ...
    if rpi_profile:
        ...
        print(f"  Memory limit: {rpi_profile.get_constraint('max_memory_mb').value}MB")
