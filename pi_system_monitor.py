...
class RaspberryPiMonitor:
    ...
    def __init__(self, log_file: str = "pi_performance.log"):
        ...
        print(f"🔍 Raspberry Pi Monitor Initialized")
  ...
  "timestamp": "2025-09-24T12:51:00.830617"
    ...
    def get_gpu_memory(self) -> Dict[str, Optional[int]]:
        ...
        try:
            ...
            result = subprocess.run(['vcgencmd', 'get_mem', 'reloc'], 
                                  capture_output=True, text=True, timeout=2)
            gpu_used = None
            if result.returncode == 0:
    ...
    def log_inference_stats(self, inference_time_ms: float, model_name: str = "unknown"):
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        temperatures = [s['temperature']['cpu_celsius'] for s in self.stats 
        ...
        summary = {
            'monitoring_duration_s': time.time() - self.start_time if self.start_time else 0,
    ...
    def print_real_time_stats(self):
        ...
        print("=" * 60)
        print(f"CPU Usage:     {latest['cpu']['percent']:6.1f}% (Load: {latest['cpu']['load_avg_1m']:.2f})")
        print(f"Memory Usage:  {latest['memory']['percent']:6.1f}% ({latest['memory']['used_gb']:.2f}GB/{latest['memory']['total_gb']:.2f}GB)")
        ...
        # Per-core CPU usage
        core_usage = " | ".join([f"Core{i}: {usage:4.1f}%" 
    ...
    try:
        ...
        while True:
            ...
            if len(monitor.stats) > 10:
                ...
                print(f"   Avg CPU: {summary['system_performance']['cpu']['avg_percent']:.1f}%")
                print(f"   Avg Memory: {summary['system_performance']['memory']['avg_percent']:.1f}%")
                
                if 'temperature' in summary['system_performance']:
                    print(f"   Avg Temp: {summary['system_performance']['temperature']['avg_celsius']:.1f}°C")
        ...
        if 'inference_performance' in summary:
            ...
            print(f"Avg Inference Time: {summary['inference_performance']['avg_time_ms']:.2f}ms")
            print(f"Avg FPS: {summary['inference_performance']['avg_fps']:.2f}")
        
        print(f"Avg CPU Usage: {summary['system_performance']['cpu']['avg_percent']:.1f}%")
        print(f"Max CPU Usage: {summary['system_performance']['cpu']['max_percent']:.1f}%")
        print(f"Avg Memory Usage: {summary['system_performance']['memory']['avg_percent']:.1f}%")
        
        if 'temperature' in summary['system_performance']:
            print(f"Avg Temperature: {summary['system_performance']['temperature']['avg_celsius']:.1f}°C")
            print(f"Max Temperature: {summary['system_performance']['temperature']['max_celsius']:.1f}°C")
    ...
    try:
        ...
        for i in range(num_inferences):
            ...
            if (i + 1) % 10 == 0:
                ...
                print(f"   Progress: {i+1}/{num_inferences} | Recent avg: {recent_avg:.2f}ms")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Raspberry Pi System Monitor for EdgeFlow")
    parser.add_argument("--htop", action="store_true", help="Run htop-like real-time monitor")
    parser.add_argument("--model", help="Monitor specific model inference")
    parser.add_argument("--inferences", type=int, default=100, help="Number of inferences to run")
