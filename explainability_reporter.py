...
class EdgeFlowExplainabilityReporter:
    ...
    def generate_explainability_report(
        ...
        report_sections.append(self._explain_optimizations(config, optimization_results))
        ...
        # Recommendations
        report_sections.append(self._generate_recommendations(config, optimization_results))
...
your specific deployment scenario."""
    ...
    def _analyze_configuration(self, config: Dict[str, Any]) -> str:
        ...
        analysis.append(f"- {device_constraints.get(device, 'Unknown device characteristics')}")
        
        # Optimization strategy
        analysis.append(f"\n**Optimization Goal:** {optimize_for}")
        if optimize_for == "latency":
            analysis.append("- Focus: Minimize inference time through quantization and fusion")
        elif optimize_for == "memory":
            analysis.append("- Focus: Minimize memory usage through aggressive quantization and pruning")
        elif optimize_for == "size":
            analysis.append("- Focus: Minimize model size through quantization and structured pruning")
        else:
    ...
    def _explain_optimizations(self, config: Dict[str, Any], results: Dict[str, Any]) -> str:
        ...
        explanations.append("The following optimizations were applied based on your configuration:\n")
    ...
    def _analyze_performance_impact(self, comparison: Dict[str, Any]) -> str:
        ...
        if size_reduction > 0:
            ...
            if size_reduction > 50:
                analysis.append("- Excellent size reduction achieved through aggressive optimization")
            elif size_reduction > 25:
                analysis.append("- Good size reduction, model significantly more deployable")
            else:
                analysis.append("- Modest size reduction, consider additional optimizations")
        ...
        if memory_improvement > 0:
            ...
            analysis.append("- Reduced memory footprint enables deployment on constrained devices")
        ...
        if throughput_improvement > 0:
            ...
            analysis.append("- Higher throughput enables processing more data per second")
    ...
    def _explain_device_optimizations(self, config: Dict[str, Any]) -> str:
        ...
        if device in device_explanations:
            explanations.append(f"**{device.replace('_', ' ').title()} Optimizations:**")
    ...
    def _generate_recommendations(self, config: Dict[str, Any], results: Dict[str, Any]) -> str:
        ...
        if quantize == "none":
            ...
            recommendations.append("- Consider INT8 quantization for significant size and speed improvements")
            if device in ["jetson_nano", "jetson_xavier"]:
                recommendations.append("- FLOAT16 quantization may provide good balance on GPU-enabled devices")
        
        # Device-specific recommendations
        if device == "raspberry_pi":
            recommendations.append("\n### Raspberry Pi Specific")
            recommendations.append("- Use structured pruning to maintain hardware efficiency")
            recommendations.append("- Consider reducing model complexity if memory is constrained")
            
        elif device in ["cortex_m4", "cortex_m7"]:
            recommendations.append("\n### Microcontroller Specific")
            recommendations.append("- Ensure all operations are integer-only for Cortex-M4")
            recommendations.append("- Consider model compression techniques for tight memory constraints")
        
        # Memory recommendations
        if memory_limit and memory_limit < 256:
            recommendations.append("\n### Memory Optimization")
            recommendations.append("- Consider more aggressive pruning for very tight memory constraints")
        ...
        if size_reduction < 25:
            ...
            recommendations.append("- Model may benefit from additional quantization or pruning")
