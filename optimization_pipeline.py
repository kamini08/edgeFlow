...
from semantic_validator import SemanticValidator, Diagnostic
...
class OptimizationMetrics:
    ...
    def size_reduction_ratio(self) -> float:
        return (self.original_size_mb - self.optimized_size_mb) / self.original_size_mb if self.original_size_mb > 0 else 0.0
    
    @property
    def speedup_ratio(self) -> float:
        return self.original_latency_ms / self.optimized_latency_ms if self.optimized_latency_ms > 0 else 1.0
...
class OptimizationPipeline:
    ...
    def __init__(self, device_profile: Optional[DeviceProfile] = None):
        ...
        self.device_profile = device_profile or get_profile_manager().get_default_profile()
    ...
    def optimize(
        ...
        try:
            ...
            if optimization_config.get("quantization", {}).get("enabled", False):
                ...
                optimized_model = self._apply_quantization(optimized_model, optimization_config["quantization"])
            ...
            if optimization_config.get("fusion", {}).get("enabled", True):
                ...
                optimized_model = self._apply_fusion(optimized_model, optimization_config.get("fusion", {}))
            
            # 6. Apply memory optimizations if enabled
            if optimization_config.get("memory_optimization", {}).get("enabled", False):
                self._log_stage(OptimizationStage.MEMORY_OPTIMIZATION, "Applying memory optimizations")
                optimized_model = self._optimize_memory(optimized_model, optimization_config["memory_optimization"])
            
            # 7. Compile for target device
            self._log_stage(OptimizationStage.COMPILATION, "Compiling for target device")
            compiled_model = self._compile_for_device(optimized_model, self.device_profile)
            
            # 8. Validate optimized model
            self._log_stage(OptimizationStage.VALIDATION_POST_OPTIMIZATION, "Validating optimized model")
            ...
            self.diagnostics.append(
                Diagnostic(
                    ...
                    context={"exception": str(e), "stage": getattr(self, "current_stage", "unknown")}
                )
            )
            return {
                "success": False,
    ...
    def _validate_inputs(self, model_path: Path, config: Dict[str, Any]) -> None:
        ...
        if any(d.severity == "error" for d in self.diagnostics):
            error_messages = [d.message for d in self.diagnostics if d.severity == "error"]
            raise ValueError(f"Invalid optimization configuration: {', '.join(error_messages)}")
    ...
    def _load_model(self, model_path: Path) -> Any:
        ...
        # This is a simplified version - actual implementation would handle different formats
...
def create_optimization_pipeline(device_profile: Optional[DeviceProfile] = None) -> OptimizationPipeline:
