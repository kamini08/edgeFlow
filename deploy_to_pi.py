...
def deploy_to_raspberry_pi(pi_ip, model_path, pi_user="pi"):
    ...
    inference_script = "deployment/edgeflow_raspberry_pi_deployment/inference_code/inference.py"
    if Path(inference_script).exists():
        run_command(f"scp {inference_script} {pi_user}@{pi_ip}:/home/{pi_user}/edgeflow/")
    else:
        ...
        if Path("generated/inference.py").exists():
            run_command(f"scp generated/inference.py {pi_user}@{pi_ip}:/home/{pi_user}/edgeflow/")
        else:
...
"""
    ...
    run_command(f"scp /tmp/test_edgeflow.py {pi_user}@{pi_ip}:/home/{pi_user}/edgeflow/")
    run_command(f"ssh {pi_user}@{pi_ip} 'chmod +x /home/{pi_user}/edgeflow/test_edgeflow.py'")
    
    # Step 6: Test the deployment
    print("\n🧪 Step 6: Testing deployment...")
    test_result = run_command(f"ssh {pi_user}@{pi_ip} 'cd /home/{pi_user}/edgeflow && python3 test_edgeflow.py'", check=False)
    ...
    print(f"   Run inference: python3 inference.py --model models/{model_name} --benchmark")
    
    print("\n🎉 Deployment completed!")

def main():
    parser = argparse.ArgumentParser(description="Deploy EdgeFlow model to Raspberry Pi")
