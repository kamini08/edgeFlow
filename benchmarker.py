...
class EdgeFlowBenchmarker:
    ...
    def _simulate_benchmark(self, model_path: str, model_size_mb: float) -> Dict[str, Any]:
        ...
        throughput_fps = specs['base_throughput_fps'] / size_factor * throughput_multiplier
    ...
    def _calculate_improvements(self, original: Dict[str, Any], optimized: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if original['latency_ms'] > 0:
            latency_improvement = ((original['latency_ms'] - optimized['latency_ms']) / 
        ...
        if original['throughput_fps'] > 0:
            throughput_improvement = ((optimized['throughput_fps'] - original['throughput_fps']) / 
                                    original['throughput_fps']) * 100
            improvements['throughput_improvement_percent'] = round(throughput_improvement, 1)
        
        # Memory improvements
        if original['memory_usage_mb'] > 0:
            memory_improvement = ((original['memory_usage_mb'] - optimized['memory_usage_mb']) / 
                                original['memory_usage_mb']) * 100
    ...
    def _generate_summary(self, improvements: Dict[str, Any]) -> str:
        ...
        if 'size_reduction_percent' in improvements:
            summary_parts.append(f"Size reduced by {improvements['size_reduction_percent']:.1f}%")
        
        if 'latency_improvement_percent' in improvements:
            if improvements['latency_improvement_percent'] > 0:
                summary_parts.append(f"Latency improved by {improvements['latency_improvement_percent']:.1f}%")
            else:
                summary_parts.append(f"Latency increased by {abs(improvements['latency_improvement_percent']):.1f}%")
        
        if 'throughput_improvement_percent' in improvements:
            if improvements['throughput_improvement_percent'] > 0:
                summary_parts.append(f"Throughput improved by {improvements['throughput_improvement_percent']:.1f}%")
            else:
                summary_parts.append(f"Throughput decreased by {abs(improvements['throughput_improvement_percent']):.1f}%")
