...
class ShapeCompatibilityValidator(UIRValidator):
    ...
    def _validate_conv2d_shapes(
        ...
        if input_tensor.shape.rank < 3:
            issues.append(
                f"Conv2D node {node.node_id} requires input with at least 3 "
                f"dimensions, got {input_tensor.shape.rank}"
            )
        ...
        if input_tensor.shape.rank >= 3:
            ...
            if any(dim == 0 for dim in spatial_dims if isinstance(dim, int)):
                issues.append(
                    f"Conv2D node {node.node_id} has zero spatial dimensions: "
                    f"{spatial_dims}"
                )
    ...
    def _validate_dense_shapes(
        ...
        if input_tensor.shape.rank < 1:
            issues.append(
                f"Dense node {node.node_id} requires input with at least 1 "
                f"dimension, got {input_tensor.shape.rank}"
            )
    ...
    def _validate_pooling_shapes(
        ...
        if input_tensor.shape.rank < 3:
            issues.append(
                f"Pooling node {node.node_id} requires input with at least 3 "
                f"dimensions, got {input_tensor.shape.rank}"
            )
    ...
    def _validate_elementwise_shapes(
        ...
        if not self._are_shapes_broadcastable(shapes):
            issues.append(
                f"Element-wise node {node.node_id} has incompatible input shapes: "
                f"{[str(s) for s in shapes]}"
            )
    ...
    def _validate_concat_shapes(
        ...
        if len(set(ranks)) > 1:
            issues.append(
                f"Concat node {node.node_id} requires all inputs to have the same "
                f"rank, got: {ranks}"
            )
            ...
            for node_id, node in graph.nodes.items():
                if node.operation_type in unsupported_ops:
                    issues.append(
                        ...
                        f"{node.operation_type.value} for device {target_device}"
                    )
        ...
        for tensor in input_tensors:
            if tensor.shape.rank >= 3:  # Assume spatial dimensions
                ...
                if max_batch_size and tensor.shape.dimensions[0] > max_batch_size:
                    issues.append(
                        f"Input tensor {tensor.name} batch size "
                        f"{tensor.shape.dimensions[0]} exceeds device limit "
                        f"{max_batch_size}"
                    )
                ...
                if max_input_size and len(max_input_size) >= 2:
                    ...
                    if (
                        ...
                        issues.append(
                            f"Input tensor {tensor.name} spatial dimensions "
                            f"{spatial_dims} exceed device limit {max_input_size[:2]}"
                        )
    ...
    def _get_unsupported_data_types(self, graph: UIRGraph) -> List[str]:
        ...
        for tensor_name, tensor in graph.tensors.items():
            if tensor.dtype in unsupported_dtypes:
                issues.append(
                    f"Tensor {tensor_name} uses unsupported data type "
                    f"{tensor.dtype.value} for edge deployment"
                )
        ...
        for node_id, node in graph.nodes.items():
            if node.operation_type in [
                ...
                if len(input_tensors) >= 2:
                    ...
                    if len(set(dtypes)) > 1:
                        issues.append(
                            f"Node {node_id} has input tensors with incompatible "
                            f"data types: {[d.value for d in dtypes]}"
                        )
                            ...
                            for bn_output in bn_outputs:
                                ...
                                for bn_consumer in bn_consumers:
                                    ...
                                    if (
                                        ...
                                        issues.append(
                                            f"Fusion opportunity: Conv2D ({node_id}) "
                                            f"+ BatchNorm ({consumer_nodes[0]}) + "
                                            f"ReLU ({bn_consumer}) can be fused for "
            ...
            if node.operation_type == OperationType.RESHAPE:
                ...
                if input_tensors and output_tensors:
                    ...
                    if input_shape.dimensions == output_shape.dimensions:
                        issues.append(
                            f"Redundant reshape operation {node_id}: input and "
                            f"output shapes are identical"
                        )
    ...
    def _check_memory_patterns(self, graph: UIRGraph) -> List[str]:
        ...
        for node_id, node in graph.nodes.items():
            if node.operation_type in high_memory_ops:
                issues.append(
                    f"High memory operation {node.operation_type.value} at node "
                    f"{node_id} may cause memory pressure on edge devices"
                )
