...
class MemoryLimitStatement(Statement):
    ...
    limit_mb: ConstrainedInt = field(
        default_factory=lambda: ConstrainedInt(64, 16, 10000)
    )
...
class PipelineStatement(Statement):
    """Represents a pipeline statement: pipeline: { preprocess, inference,
    postprocess }
    """
...
class Conv2DDeclaration(LayerDeclaration):
    ...
    def validate_parameters(self) -> List[str]:
        ...
        if self.filters.value > 2048:
            errors.append(
                f"Conv2D filters {self.filters.value} exceeds recommended maximum "
                f"(2048)"
            )

        if isinstance(self.kernel_size.size, int) and self.kernel_size.size > 11:
            errors.append(
                f"Conv2D kernel size {self.kernel_size.size} may be too large for edge "
                f"devices"
            )
...
class DenseDeclaration(LayerDeclaration):
    ...
    def validate_parameters(self) -> List[str]:
        ...
        if self.units.value > 4096:
            errors.append(
                f"Dense units {self.units.value} may exceed memory limits on edge "
                f"devices"
            )
...
class DropoutDeclaration(LayerDeclaration):
    ...
    def validate_parameters(self) -> List[str]:
        ...
        if self.rate.rate > 0.7:
            errors.append(
                f"Dropout rate {self.rate.rate} is very high and may hurt model "
                f"performance"
            )
...
class ASTVisitor(ABC):
    ...
    @abstractmethod
    def visit_hybrid_optimization_statement(
        self, node: HybridOptimizationStatement
    ) -> Any:
        pass
...
def create_program_from_dict(config: Dict[str, Any]) -> Program:
    ...
    if "memory_limit" in config:
        statements.append(
            MemoryLimitStatement(
                limit_mb=ConstrainedInt(config["memory_limit"], 16, 10000)
            )
        )
    ...
    if "enable_hybrid_optimization" in config:
        statements.append(
            HybridOptimizationStatement(enabled=config["enable_hybrid_optimization"])
        )

    if "pytorch_quantize" in config:
        statements.append(
            PyTorchQuantizeStatement(quantize_type=config["pytorch_quantize"])
        )
...
def print_ast(node: ASTNode, indent: int = 0) -> str:
    ...
    if isinstance(node, Program):
        ...
        result.append(
            f"{prefix}PyTorchQuantizeStatement(quantize_type='{node.quantize_type}')"
        )
