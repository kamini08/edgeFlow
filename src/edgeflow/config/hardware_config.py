...
class HardwareConfig:
    ...
    def validate_model_compatibility(
        ...
        if (
            device_spec.device_type.value == "coral_dev_board"
        ...
        device_warnings = self._check_device_specific_constraints(device_spec, config)
    ...
    def get_device_recommendations(
        self, model_size_mb: float, requirements: Dict[str, Any]
    ) -> List[Tuple[str, str]]:
        """
        ...
        recommendations.sort(
            key=lambda x: self.device_manager.devices[x[0]].max_model_size_mb
            - model_size_mb,
            reverse=True,
        )
    ...
    def get_optimization_suggestions(
        self, target_device: str, current_config: Dict[str, Any]
    ) -> List[str]:
        """
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        print(
            "  validate <model_path> <device> <config_file> - Validate model for device"
        )
    ...
    if command == "export":
        ...
        if len(sys.argv) < 5:
            print(
                "Usage: python hardware_config.py validate <model_path> <device> <config_file>"
            )
            sys.exit(1)
        ...
        if len(sys.argv) < 3:
            print(
                "Usage: python hardware_config.py recommend <model_size_mb> [gpu=true] [tpu=true] [low_power=true]"
            )
            sys.exit(1)
        ...
        # Test validation
        test_config = {"quantize": "int8", "memory_limit": 64, "enable_pruning": True}
        ...
        print(
            f"✓ Validation test: Compatible={is_compatible}, Warnings={len(warnings)}"
        )
