...
class GPUTestingFramework:
    ...
    def run_comprehensive_gpu_tests(
        self, target_gpu: Optional[GPUInfo] = None
    ) -> GPUTestSuite:
        ...
        with trace_transformation(
            ...
            if target_gpu:
                logger.info(
                    f"🎯 Target GPU: {target_gpu.name} ({target_gpu.vendor.value})"
                )
            else:
            ...
            suite.summary_metrics = self._generate_summary_metrics(
                suite.test_results, target_gpu
            )
            ...
            logger.info(
                f"✅ Test suite completed: {suite.passed_tests}/{suite.total_tests} passed ({suite.success_rate:.1f}%)"
            )
    ...
    def _run_single_test(
        self, test_name: str, category: TestCategory, test_func
    ) -> TestResult:
    ...
    def _test_basic_compute_performance(self, gpu: GPUInfo) -> Dict[str, Any]:
        ...
        return {
            ...
            "metrics": {"compute_score": compute_score, **benchmarks},
    ...
    def _test_optimization_strategy(
        self, gpu: GPUInfo, strategy: GPUOptimizationStrategy
    ) -> Dict[str, Any]:
        """Test GPU optimization strategy."""
        try:
            # Create optimization config
            config = self.gpu_integrator.create_gpu_optimization_config(
                "auto", strategy
            )
    ...
    def _test_repeated_operations(self, gpu: GPUInfo) -> Dict[str, Any]:
        ...
        return {
            ...
            "metrics": {"iterations": iterations, "success_rate": success_rate},
    ...
    def _generate_summary_metrics(
        self, test_results: List[TestResult], gpu: Optional[GPUInfo]
    ) -> Dict[str, Any]:
        ...
        for category in TestCategory:
            ...
            if category_tests:
                success_rate = sum(1 for r in category_tests if r.success) / len(
                    category_tests
                )
        ...
        if gpu:
            if metrics.get("performance", 0) < 70:
                metrics["recommendations"].append(
                    "Consider GPU driver updates for better performance"
                )
            ...
            if gpu.memory_mb < 2048:
                metrics["recommendations"].append(
                    "Limited GPU memory may require model optimization"
                )
    ...
    def export_test_report(
        self, suite: GPUTestSuite, output_file: str = "gpu_test_report.json"
    ) -> None:
...
if __name__ == "__main__":
    ...
    if suite.gpu_info:
        ...
        print(
            f"   Overall health score: {suite.summary_metrics.get('overall_health_score', 0):.1f}/100"
        )
