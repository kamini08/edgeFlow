...
class ProvenanceTracker:
    ...
    def get_artifact_lineage(self, artifact_id: str) -> List[TransformationEvent]:
        ...
        for event in self.events:
            if (
                artifact_id in event.input_artifacts
                or artifact_id in event.output_artifacts
            ):
    ...
    def get_summary_statistics(self) -> Dict[str, Any]:
        ...
        warning_count = sum(
            1 for e in self.events if e.severity == SeverityLevel.WARNING
        )
        ...
        return {
            ...
            "session_duration_ms": (datetime.now() - self.start_time).total_seconds()
            * 1000,
        }
    ...
    def import_provenance_report(self, input_path: str) -> None:
        ...
        # Import events
        self.events = [
            TransformationEvent.from_dict(event_data) for event_data in report["events"]
        ]
