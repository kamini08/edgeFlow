        ...
        self.error_patterns = {
            ...
            r"Model size (.+)MB exceeds device limit (.+)MB": {
                ...
                "suggestions": [
                    ErrorSuggestion(
                        description=(
                            "Enable aggressive quantization to reduce model size"
                        ),
...
class IntegratedErrorReporter:
    ...
    def _add_context_suggestions(self, error: EdgeFlowError) -> None:
        ...
        if error.source_file and error.source_file.endswith(".ef"):
            error.suggestions.append(
                ErrorSuggestion(
                    description=(
                        f"Validate the entire file: edgeflow validate "
                        f"{error.source_file}"
                    ),
                    fix_command=(
                        f"python interactive_validator.py validate-file "
                        f"{error.source_file}"
                    ),
                    confidence=0.6,
                    automated=True,
        ...
        if error.component == "optimizer":
            error.suggestions.append(
                ErrorSuggestion(
                    ...
                    code_snippet=(
                        "optimization_level = basic  # or aggressive, balanced"
                    ),
                    confidence=0.5,
                )
            )
        ...
        if error.severity == ValidationSeverity.CRITICAL:
            error.suggestions.insert(
                ...
                ErrorSuggestion(
                    description=(
                        "This is a critical error that must be fixed before proceeding"
                    ),
                    confidence=1.0,
                ),
            )
    ...
    def _log_error(self, error: EdgeFlowError) -> None:
        ...
        log_message = (
            f"[{error.category.value.upper()}] {error.title}{location}: {error.message}"
        )
    ...
    def get_session_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "automated_fixes_available": sum(
                1 for e in self.session_errors for s in e.suggestions if s.automated
            ),
        }

    def apply_automated_fixes(
        self, error_ids: Optional[List[str]] = None
    ) -> Dict[str, bool]:
        ...
        if error_ids:
            errors_to_fix = [
                self.error_history[eid]
                for eid in error_ids
                if eid in self.error_history
            ]
        else:
            errors_to_fix = [
                e
                for e in self.session_errors
                if any(s.automated for s in e.suggestions)
            ]
                ...
                if suggestion.automated and suggestion.fix_command:
                    try:
                        ...
                        if success:
                            ...
                            logger.warning(
                                f"Automated fix failed for {error.error_id}: "
                                f"{result.stderr}"
                            )
                        ...
                        logger.error(
                            f"Failed to apply automated fix for {error.error_id}: {e}"
                        )
    ...
    def generate_error_report(self, output_file: str = "error_report.md") -> str:
        ...
        report_lines.extend(["", "## Error Summary by Severity", ""])
        ...
        if summary["critical_errors"]:
            report_lines.extend(["", "## Critical Errors (Must Fix)", ""])
        ...
        # All other errors
        other_errors = [
            e for e in self.session_errors if e.severity != ValidationSeverity.CRITICAL
        ]
        if other_errors:
            report_lines.extend(["", "## Other Errors and Warnings", ""])
    ...
    def _format_error_for_report(self, error: EdgeFlowError) -> List[str]:
        ...
        lines.extend(["", f"**Message:** {error.message}", ""])
        ...
        if error.suggestions:
            ...
            for i, suggestion in enumerate(error.suggestions, 1):
                ...
                if suggestion.code_snippet:
                    lines.extend(["   ```", f"   {suggestion.code_snippet}", "   ```"])
                ...
                if suggestion.documentation_url:
                    lines.append(
                        f"   **Documentation:** {suggestion.documentation_url}"
                    )
