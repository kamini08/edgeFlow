...
class CLIFormatter:
    ...
    def __init__(self, use_colors: bool = True, use_icons: bool = False):
        ...
        Args:
            ...
            use_icons: Whether to use text indicators (disabled by default for
                professional look)
        """
    ...
    def header(self, text: str, level: int = 1) -> str:
        ...
        if level == 1:
            ...
            formatted = (
                ...
                f"{self.colorize(text, Color.BRIGHT_MAGENTA)}"
            )
    ...
    def format_stats(self, stats: Dict[str, Any], title: Optional[str] = None) -> str:
        ...
        for key, value in stats.items():
            ...
            line = (
                f"  {self.colorize(formatted_key, Color.BRIGHT_WHITE)}: "
                f"{self.colorize(formatted_value, color)}"
            )
            lines.append(line)
    ...
    def format_comparison(
            ...
            if key in after:
                ...
                if before_val > 0:
                    if "size" in key.lower() or "memory" in key.lower():
                        ...
                        if improvement > 0:
                            improvements.append(
                                (key, before_val, after_val, improvement)
                            )
        ...
        for metric, before_val, after_val, improvement in improvements:
            ...
            improvement_str = self.colorize(
                f"({improvement:.1f}% improvement)", Color.GREEN
            )

            line = (
                f"  {formatted_metric}: {before_str} {arrow} {after_str} "
                f"{improvement_str}"
            )
            lines.append(line)
...
class Spinner:
    ...
    def __init__(
        self, description: str = "Processing", formatter: Optional[CLIFormatter] = None
    ):
        """Initialize spinner.
...
def create_summary_box(
    title: str, content: List[str], formatter: Optional[CLIFormatter] = None
) -> str:
    ...
    # Top border
    lines.append(
        formatter.colorize("╔" + "═" * (box_width - 2) + "╗", Color.BRIGHT_BLUE)
    )
