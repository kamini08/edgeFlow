...
def generate_report(
    ...
    unopt_fps = (
        1000 / float(unoptimized_stats["latency_ms"])
        if unoptimized_stats["latency_ms"] > 0
        else 0
    )
    opt_fps = (
        1000 / float(optimized_stats["latency_ms"])
        if optimized_stats["latency_ms"] > 0
        else 0
    )
...
| **Throughput** | {unopt_fps:.1f} fps | {opt_fps:.1f} fps | \
