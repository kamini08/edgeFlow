...
from .error_types import ErrorCollector, ErrorSeverity, ErrorType, SemanticError
from .ir_nodes import IRGraph, IRNode, LayerType
                ...
                if not input_node:
                    self.error_collector.add_error(
                        SemanticError(
                            ...
                            severity=ErrorSeverity.ERROR,
                            message=(
                                f"Referenced input node '{input_node_id}' "
                                f"does not exist"
                            ),
                            node_id=node.node_id,
                            layer_name=node.display_name,
                ...
                if not output_node:
                    self.error_collector.add_error(
                        SemanticError(
                            ...
                            severity=ErrorSeverity.ERROR,
                            message=(
                                f"Referenced output node '{output_node_id}' "
                                f"does not exist"
                            ),
                            node_id=node.node_id,
                            layer_name=node.display_name,
        ...
        if producer_output.dtype != consumer_input.dtype:
            self.error_collector.add_error(
                SemanticError(
                    ...
                    severity=ErrorSeverity.WARNING,
                    message=(
                        f"Datatype mismatch: producer outputs "
                        f"{producer_output.dtype.value}, "
                        ...
                        self.error_collector.add_error(
                            SemanticError(
                                ...
                                message=(
                                    ...
                                    f"not in valid values {param_range.valid_values}"
                                ),
                                node_id=node.node_id,
                                layer_name=node.display_name,
                                location=node.location,
                ...
                if actual_rank not in expected_ranks:
                    self.error_collector.add_error(
                        SemanticError(
                            ...
                            severity=ErrorSeverity.ERROR,
                            message=(
                                f"Layer expects input rank {expected_ranks}, "
                                f"got {actual_rank}"
                            ),
                            node_id=node.node_id,
                            layer_name=node.display_name,
                    ...
                    if prev_node.layer_type in [
                        ...
                        self.error_collector.add_error(
                            SemanticError(
                                ...
                                severity=ErrorSeverity.ERROR,
                                message=(
                                    "Dense layer requires Flatten layer after "
                                    f"{prev_node.layer_type.value}"
                                ),
                                node_id=node.node_id,
                                layer_name=node.display_name,
                                location=node.location,
                                suggestion=(
                                    "Add a Flatten layer between the convolutional "
                                    "and dense layers"
                                ),
                            )
                        )
            ...
            if curr_node and next_node:
                if self.config.is_sequence_forbidden(
                    ...
                    self.error_collector.add_forbidden_config(
                        config_description=(
                            f"{curr_node.layer_type.value} followed by "
                            f"{next_node.layer_type.value}"
                        ),
                        node_id=next_node.node_id,
                        layer_name=next_node.display_name,
                        location=next_node.location,
                        suggestion=(
                            f"Add appropriate intermediate layers between "
                            f"{curr_node.layer_type.value} and "
                            f"{next_node.layer_type.value}"
                        ),
                    )
        ...
        for forbidden_combo in self.config.forbidden_combinations:
            if forbidden_combo["condition"](graph):
                self.error_collector.add_forbidden_config(
                    ...
                    suggestion=(
                        "Review the model architecture and pipeline configuration"
                    ),
                )
            ...
            if node.layer_type not in device_constraints.supported_layers:
                self.error_collector.add_error(
                    SemanticError(
                        ...
                        severity=ErrorSeverity.ERROR,
                        message=(
                            f"Layer type {node.layer_type.value} "
                            "is not supported on target device"
                        ),
                        node_id=node.node_id,
                        layer_name=node.display_name,
                        location=node.location,
                ...
                if tensor.dtype not in device_constraints.supported_dtypes:
                    self.error_collector.add_error(
                        SemanticError(
                            ...
                            severity=ErrorSeverity.ERROR,
                            message=(
                                f"Data type {tensor.dtype.value} is not supported "
                                "on target device"
                            ),
                            node_id=node.node_id,
                            layer_name=node.display_name,
                            ...
                            suggestion=(
                                ...
                                f"{[dt.value for dt in device_constraints.supported_dtypes]}"  # noqa: E501
                            ),
                        )
                    )
