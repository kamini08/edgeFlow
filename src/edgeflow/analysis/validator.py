...
class EdgeFlowValidator:
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.debug(
                "Hardware configuration system not available, using basic validation"
            )
    ...
    def _validate_model_file(self, config: Dict[str, Any], errors: List[str]) -> None:
        ...
        try:
            ...
            if self.use_hardware_config:
                try:
                    device_spec = self.hardware_config.device_manager.get_device_spec(
                        device
                    )
                    if device_spec:
                    ...
                    max_model_size = self.device_constraints.get(
            ...
            try:
                ...
                if not device_spec:
                    ...
                    available_devices = list(
                        self.hardware_config.device_manager.devices.keys()
                    )
                ...
                logger.warning(
                    f"Hardware config validation failed, falling back to basic validation: {e}"
                )
        ...
        try:
            ...
            if self.use_hardware_config and os.path.exists(model_path):
                try:
                    (
                        is_compatible,
                        hw_warnings,
                    ) = self.hardware_config.validate_model_compatibility(
                    ...
                    logger.warning(
                        f"Hardware config validation failed, using basic validation: {e}"
                    )
                    # Fall back to basic validation
            ...
            if os.path.exists(model_path):
                ...
                if self.use_hardware_config:
                    try:
                        device_spec = (
                            self.hardware_config.device_manager.get_device_spec(device)
                        )
                        if device_spec:
                            max_model_size = device_spec.max_model_size_mb
                        else:
                ...
                if model_size_mb > max_model_size:
                    return False, [
                        f"Model too large for {device}: {model_size_mb:.1f}MB > {max_model_size}MB"
                    ]
