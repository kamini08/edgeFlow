...
class ValidationResult:
    ...
    def get_warning_count(self) -> int:
        ...
        return sum(
            1 for msg in self.messages if msg.severity == ValidationSeverity.WARNING
        )
...
class InteractiveValidator:
        ...
        with trace_transformation(
            ...
            if show_progress:
                with Progress(
                    ...
                    progress.update(
                        task, advance=30, description="Validating parameters..."
                    )
                    ...
                    if target_device:
                        progress.update(
                            task,
                            advance=25,
                            description="Checking device compatibility...",
                        )
                        device_messages = self._validate_device_compatibility(
                    ...
                    progress.update(
                        task, advance=25, description="Generating performance hints..."
                    )
                    perf_messages = self._generate_performance_hints(
                        file_path, target_device
                    )
                    result.messages.extend(perf_messages)

                    progress.update(
                        task, completed=100, description="Validation complete!"
                    )
            else:
                ...
                if target_device:
                    result.messages.extend(
                        self._validate_device_compatibility(file_path, target_device)
                    )
                result.messages.extend(
                    self._generate_performance_hints(file_path, target_device)
                )
    ...
    def validate_directory(
        ...
        with Progress(
            ...
            main_task = progress.add_task(
                "Validating directory...", total=len(ef_files)
            )
            ...
            for ef_file in ef_files:
                ...
                file_result = self.validate_file(
                    str(ef_file), target_device, show_progress=False
                )
    ...
    def _validate_device_compatibility(
        self, file_path: str, target_device: str
    ...
    def _generate_performance_hints(
        self, file_path: str, target_device: Optional[str]
    ) -> List[ValidationMessage]:
        ...
        try:
            ...
            if config.get("quantize", "none") == "none" and target_device in [
                "raspberry_pi",
                "edge",
                "mobile",
            ]:
...
def check(ctx, file_path, watch):
    ...
    if watch:
        console.print(
            f"👀 Watching file: [bold]{file_path}[/bold] (Press Ctrl+C to stop)"
        )
