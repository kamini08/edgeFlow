...
class EdgeFlowOptimizer:
    ...
    def apply_pruning(self, model, pruning_params: Dict[str, Any]):
        ...
        if not self.tf_available or not self.tfmot_available:
            logger.warning(
                "TensorFlow Model Optimization not available, skipping pruning"
            )
            return model
    ...
    def convert_pytorch_to_onnx(
        self, pytorch_model, input_shape=(1, 3, 224, 224), onnx_path=None
    ):
    ...
    def convert_onnx_to_tensorflow(self, onnx_path):
        ...
        try:
            ...
            logger.warning(
                "ONNX to TensorFlow conversion requires additional libraries"
            )
            return None
    ...
    def optimize_hybrid_pipeline(
        ...
        if framework == "pytorch" and self.torch_available:
            ...
            input_shape = tuple(
                int(x) for x in config.get("input_shape", "1,3,224,224").split(",")
            )
            ...
            if onnx_path and self.onnx_available:
                ...
                if tf_model:
                    ...
                    logger.info(
                        ...
                        "(TensorFlow conversion not available)"
                    )
                    return onnx_path, {
                        "framework": "onnx",
            ...
            logger.warning(
                f"Unsupported framework {framework}, using standard pipeline"
            )
            return self._optimize_tensorflow_standard(config)
    ...
    def optimize_tensorflow_model(
        self, tf_model, config: Dict[str, Any]
    ) -> Tuple[str, Dict[str, Any]]:
    ...
    def optimize_model(self, config: Dict[str, Any]) -> Tuple[str, Dict[str, Any]]:
        """Optimize a model using real TensorFlow Lite quantization, pruning,
        and operator fusion..
    ...
    def _optimize_tensorflow_standard(
        self, config: Dict[str, Any]
    ) -> Tuple[str, Dict[str, Any]]:
        """Standard TensorFlow optimization pipeline."""
        ...
        try:
            ...
            if keras_source and (
                ...
                try:
                    ...
                    logger.warning(
                        "This is likely due to TensorFlow version mismatch "
                        "or custom layers."
                    )
                    logger.warning(
                        "Falling back to basic TFLite optimization "
                ...
                if enable_pruning:
                    pruning_params = {
                        ...
                        "structured": True,  # Use structured pruning for better
                        # hardware
                    }
            ...
            if (
                ...
                logger.warning(
                    "Quantization requested (%s) but no keras_model provided; "
                    "cannot quantize existing TFLite model. "
                    "Applying basic optimizations instead.",
                    quantize,
                )
                # Fall back to basic optimizations on existing TFLite model
                return self._optimize_existing_tflite(config)
            ...
            if keras_source and not keras_model:
                try:
                    ...
                    logger.warning(
                        "Failed to load Keras model %s for optimization: %s",
                        keras_source,
                        load_error,
                    )
                    logger.warning(
                        "Falling back to basic TFLite optimization "
                        "without advanced quantization."
                    )
                    return self._optimize_existing_tflite(config)
            ...
            optimized_path = config.get("model", "model.tflite").replace(
                ".tflite", "_optimized.tflite"
            )
    ...
    def _optimize_existing_tflite(
        ...
        try:
            ...
            converter = self.tf.lite.TFLiteConverter.from_saved_model(
                tf.saved_model.load(model_path) if os.path.isdir(model_path) else None
            )
            ...
            if not os.path.isdir(model_path):
                ...
                # We can't actually load from TFLite file, so we'll copy with
                # basic metadata
            ...
            return optimized_path, {
                ...
                "target_device": target_device,
                "optimizations_applied": [],
                "note": "Model copied (optimizations not applicable to existing "
                "TFLite)",
            }
