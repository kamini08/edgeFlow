"""
Grammar-aware semantic validator that integrates with ANTLR parser and existing
semantic analyzer.
...
from semantic_analyzer.constraints import ConstraintConfig
...
class GrammarSemanticValidator(ASTVisitor):
    ...
    def _validate_layer_sequence(self):
        ...
        for i in range(len(self.layer_sequence) - 1):
            ...
            if self._is_forbidden_sequence(current_type, next_type):
                self._add_error(
                    f"Forbidden layer sequence: {current_type.value} -> "
                    f"{next_type.value}",
        ...
        for layer_name, layer_type in self.layer_names.items():
            ...
            if layer_type not in device_limits.supported_layers:
                self._add_error(
                    ...
                    layer_name=layer_name,
                    suggestion=f"Use supported layer types: "
                    f"{[lt.value for lt in device_limits.supported_layers]}",
                )
    ...
    def validate_conv2d_parameters(self, layer: Conv2DDeclaration) -> List[str]:
        ...
        if layer.filters.value > device_limits.max_filters:
            errors.append(
                f"Conv2D filters {layer.filters.value} exceeds device limit "
                f"{device_limits.max_filters}"
            )
        ...
        if max_kernel > device_limits.max_kernel_size:
            errors.append(
                f"Conv2D kernel size {max_kernel} exceeds device limit "
                f"{device_limits.max_kernel_size}"
            )
    ...
    def validate_dense_parameters(self, layer: DenseDeclaration) -> List[str]:
        ...
        if layer.units.value > device_limits.max_units:
            errors.append(
                f"Dense units {layer.units.value} exceeds device limit "
                f"{device_limits.max_units}"
            )
